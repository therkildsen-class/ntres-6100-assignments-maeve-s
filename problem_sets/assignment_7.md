...
q_4b <- q_4a |> 
  left_join(q_1b) |> 
  mutate(polling_margin = c(adjpoll_clinton - adjpoll_trump)) |> 
  mutate(actual_margin = c(clinton - trump)) |> 
  mutate(polling_error = c(polling_margin - actual_margin)) |> 
  mutate(predicted_winner = ifelse(polling_margin > 0, "Clinton", "Trump")) |> 
